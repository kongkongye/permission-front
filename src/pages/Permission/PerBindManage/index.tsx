import { addPerBind, delPerBind, listPerBindBriefs, listPerTypes } from '@/services/permission/api';
import {
  ProFormGroup,
  ProFormSelect,
  ProFormSwitch,
...
const PerManage: React.FC = () => {
  ...
  const selNodeCallback = useCallback((type: string, ...args: any[]) => {
    ...
    if (type === 'dept') {
      ...
      const dept = args[1];
      bindNode = `[部门]${
        routes && routes.length > 0 ? routes.map((e: any) => e.name).join('>') : ''
      }`;
      setSelNode({
        type: 'dept',
        code: dept.code,
  ...
  return (
    <Layout>
      <Content>
        ...
        />
        <BindNodeSel
          visible={bindNodeVisible}
          onSel={selNodeCallback}
          onClose={() => setBindNodeVisible(false)}
        />
      </Content>
    </Layout>
  );
